...
if len(plot_df) > 0:
    ...
    ax.barh(events, deltas, color=colors, alpha=0.8, height=0.5)
    ax.axvline(0, color="black", linewidth=0.8)
    ax.set_xlabel("Δ (days)", fontsize=9)
    ax.tick_params(axis="both", labelsize=9)
    ax.set_ylabel("")
    plt.tight_layout()
    fig.savefig("Figures/Fig04_timing_lead_lag.png", dpi=300, bbox_inches="tight")
    fig.savefig("Figures/Fig04_timing_lead_lag.svg", bbox_inches="tight")
    plt.close()
